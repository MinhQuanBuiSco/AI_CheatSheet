...
class SFTConfig:
    ...
    # Optimization (auto-adjusted based on device capabilities)
    fp16: bool = True
...
class ProductionSFTTrainer:
    ...
    def __init__(self, config: SFTConfig):
        ...
        # Check and adjust device-specific settings
    ...
    def check_device_compatibility(self):
        """Check device compatibility and adjust settings accordingly."""
        # Disable fp16 for MPS devices (Apple Silicon)
        if torch.backends.mps.is_available() and torch.backends.mps.is_built() and self.config.fp16:
            print("INFO: Detected MPS device (Apple Silicon). Disabling fp16 mixed precision.")
            self.config.fp16 = False
            
        # Check if CUDA is available for other optimizations
        if torch.cuda.is_available():
            print(f"INFO: CUDA available with {torch.cuda.device_count()} GPU(s)")
        elif torch.backends.mps.is_available():
            print("INFO: Using MPS (Apple Silicon) acceleration")
        else:
