...
from training.sft_trainer import SFTConfig, ProductionSFTTrainer
from training.utils.evaluation import EvaluationConfig, ComprehensiveEvaluator
...
def create_sft_config(config_dict: dict, args) -> SFTConfig:
    ...
    # Auto-detect device capabilities for fp16
    default_fp16 = config_dict.get("training", {}).get("fp16", True)
    if torch.backends.mps.is_available() and torch.backends.mps.is_built():
        # MPS doesn't support fp16 mixed precision
        if default_fp16:
            print("WARNING: fp16 not supported on MPS devices, disabling fp16")
        default_fp16 = False
